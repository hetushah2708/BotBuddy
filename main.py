...
def start_listening():
    try:
        recognizer = sr.Recognizer()
        # Optional: make recognizer more sensitive
        recognizer.energy_threshold = 150  # Lower value = more sensitive, adjust as needed
        with sr.Microphone() as source:
            app.logger.info("Listening from server microphone...")
            try:
                # Listen for up to 5 seconds after speech starts, and wait up to 4 seconds for speech to start
                audio = recognizer.listen(source, timeout=4, phrase_time_limit=5)
            except WaitTimeoutError:
                app.logger.error("No speech detected within the timeout window.")
